...
impl Function for Div {
    ...
    fn backward(&mut self, grad_outputs: Vec<Cell<Tensor>>) -> Vec<Tensor> {
        let (grad_a, grad_b) = {
            let a = &self.left.borrow_mut().data;
            let b = &self.right.borrow_mut().data;
            let g = &grad_outputs.get(0).unwrap().borrow_mut().data;
            (g / b, g * (a * -1.0) / b.pow2())
        };
        let binding = self.right.borrow_mut();
        ...
        if grad_b.shape().len() == 1 && grad_b.shape() != b_shape {
            ...
            let grad_b = Tensor {
                data: ArcArray::from_iter([grad_b.borrow().sum()]).into_dyn().into(),
            ...
            assert_eq!(self.left.borrow_mut().data.shape(), grad_a.data.shape());
            assert_eq!(self.right.borrow_mut().data.shape(), grad_b.data.shape());
            vec![grad_a, grad_b]
            ...
            let g = {
                while gb.shape() != b_shape {
                    gb = gb.sum_axis(ndarray::Axis(0)).into();
                }
...
mod tests {
    ...
    fn test_mul1() {
        for _ in 0..1000 {
            let A: ndarray::ArrayBase<ndarray::OwnedRepr<f64>, ndarray::Dim<[usize; 1]>> = Array::random((7280), Uniform::new(0.01, 100.));
            let B: ndarray::ArrayBase<ndarray::OwnedRepr<f64>, ndarray::Dim<[usize; 1]>> = Array::random((7280), Uniform::new(0.01, 100.));
            ...
            let my_output = &W2 / &W;
            let mut my_output2 = my_output.log().exp().sqrt().log() / &W2;//my_output.dot(&my_output);
            let mut my_output3 = my_output2.sum();
            backward(&mut my_output3);
            ...
            let pyout = &pyW2 / &pyW;
            let pyout2 = pyout.log().exp().sqrt().log() / &pyW2;//pyout.dot(&pyout);
            let mut pyout3 = pyout2.sum(None);
